...
class RmseLoss(nn.Module):
    ...
    RMSE loss
    """
    def __init__(self, start_ind=0, end_ind=100):
...
class WeightedRmseLoss(nn.Module):
    def __init__(
        self,
        aux_data_dir="./scs_3dmodel_forecast_data/",
        weight_per_variable=False,
        start_ind=0,
        end_ind=5,
    ):
        ...
        if self.weight_per_variable:
            self.variable_weights = torch.from_numpy(
                np.load(aux_data_dir + "weights/loss_var_weights.npy")[
                    np.newaxis, np.newaxis, np.newaxis, start_ind:end_ind
                ]
            ).float()

    def forward(self, target, output, expand=False):
        # target: [B, lon, lat, 5]
        # output: [B, lon, lat, C] where C = end_ind - start_ind

        target = target[..., self.start_ind:self.end_ind].to(output.device)

        if output.shape[-1] != target.shape[-1]:
            raise ValueError(
                f"Output channels {output.shape[-1]} do not match target channels {target.shape[-1]}. "
                f"Check start_ind/end_ind and model out_channels."
            )

        squared_diff = (target - output) ** 2

        if self.weight_per_variable:
            squared_diff = squared_diff * self.variable_weights.to(output.device)

        if not expand:
            return torch.nanmean(
                torch.sqrt(torch.nanmean(squared_diff, dim=(1, 2, 3)))
            )

        return torch.mean(
            torch.sqrt(torch.nanmean(squared_diff, dim=(1, 2))),
            dim=0
        )